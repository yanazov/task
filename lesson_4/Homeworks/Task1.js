...
document.addEventListener("DOMContentLoaded", function () {
    ...
    input_mail.addEventListener("change", function () {
        if( input_mail.validity.typeMismatch){
            input_mail.setCustomValidity('Ну и зря, не получишь бандероль с яблоками!');
        } else {
            input_mail.setCustomValidity('');
        }
    });
    document.getElementById('input_password').setCustomValidity('Я никому не скажу наш секрет');
    document.getElementById('input_password').addEventListener('change', function(){
    ...
    // честное слово я понял как оно работет, дальше просто нет сил делать т.к. на проде такое всеравно не используется     
